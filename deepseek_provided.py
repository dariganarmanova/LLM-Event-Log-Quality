#!/usr/bin/env python3
"""
Parameterized Deepseek CSV Processor with Clean Logging
Usage: python deepseek_processor.py <csv_file> <prompt_file> [options]
"""
import os
import subprocess
import pandas as pd
...
class ParameterizedLlamaProcessor:
    def __init__(self, api_key=None, model_name="deepseek-ai/DeepSeek-V3-0324"):
        """Initialize the Parameterized Deepseek CSV Processor"""
        self.api_key = api_key or os.getenv('HF_TOKEN') or os.getenv('HUGGINGFACE_API_KEY')
        if not self.api_key:
            raise ValueError("Hugging Face API token not found. Please set HF_TOKEN or HUGGINGFACE_API_KEY environment variable.")
        ...
        try:
            ...
            print(f"[INFO] HuggingFace client initialized with model: {self.model_name}")
...
"""
    ...
    def generate_code(self, prompt, max_tokens=2000, temperature=0.3, max_retries=5):
        """Generate code using chat completion API"""
        for attempt in range(max_retries):
            try:
                print(f"Attempt {attempt + 1}: Trying chat completion API...")
                completion = self.client.chat.completions.create(
                    model=self.model_name,
                    messages=[
                        {
                            "role": "system",
                            "content": "You are an expert Python programmer specializing in data analysis and CSV manipulation. Generate clean, well-commented, and executable Python code. Always include proper imports and error handling."
                        },
                        {
                            "role": "user",
                            "content": prompt
                ...
                if "loading" in error_msg and attempt < max_retries - 1:
                    ...
                    continue
                elif "rate limit" in error_msg and attempt < max_retries - 1:
    ...
    def save_generated_script(self, code, csv_path, prompt_path, run_number):
        ...
        script_filename = f"{csv_basename}_Deepseek_run{run_number}.py"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Parameterized Deepseek CSV Processor")
    ...
    parser.add_argument("--api-key", help="Hugging Face API token")
    ...
    print("[INFO] Starting Parameterized Deepseek CSV Processing Pipeline")
    ...
    try:
        processor = ParameterizedLlamaProcessor(api_key=args.api_key, model_name=model_name)
        
    except ValueError as e:
        print(f"[ERROR] {e}")
        print("[INFO] Please set your Hugging Face API token as an environment variable:")
        print("[INFO] export HF_TOKEN='your-huggingface-token-here'")
        return
    except Exception as e:
...
if __name__ == "__main__":
    if len(os.sys.argv) == 1:
        print("[INFO] Parameterized Deepseek CSV Processor")
        print("[INFO] Usage: python deepseek_processor.py <csv_file> <prompt_file> [options]")
    else:
        main()