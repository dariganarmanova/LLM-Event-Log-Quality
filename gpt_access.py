#!/usr/bin/env python3
"""
Parameterized GPT CSV Processor with Clean Logging
Usage: python gpt_processor.py <csv_file> <prompt_file> [options]
"""
...
class ParameterizedGPTProcessor:
    def __init__(self, api_key=None, model_name="gpt-4o"):
        """Initialize the Parameterized GPT CSV Processor"""
...
"""
    ...
    def generate_code(self, prompt, max_tokens=2000, temperature=0.3, max_retries=5):
        """Generate code using OpenAI chat completion API"""
    ...
    def save_generated_script(self, code, csv_path, prompt_path, run_number):
        ...
        script_filename = f"{csv_basename}_GPT_run{run_number}.py"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Parameterized GPT CSV Processor")
    ...
    parser.add_argument("--model", type=str, default="gpt-4o-2024-11-20", help="GPT model to use")
    ...
    print("[INFO] Starting Parameterized GPT CSV Processing Pipeline")
    ...
    try:
        processor = ParameterizedGPTProcessor(api_key=args.api_key, model_name=model_name)
        
    except ValueError as e:
        print(f"[ERROR] {e}")
        print("[INFO] Please set your OpenAI API token as an environment variable:")
        print("[INFO] export OPENAI_API_KEY='your-openai-token-here'")
        return
    except Exception as e:
...
if __name__ == "__main__":
    if len(os.sys.argv) == 1:
        print("[INFO] Parameterized GPT CSV Processor")
        print("[INFO] Usage: python gpt_processor.py <csv_file> <prompt_file> [options]")
    else:
        main()