...
# Generated on: 2025-11-13T16:06:46.972678
...
for case in df[case_column].unique():
    ...
    while i < len(case_indices):
        ...
        if len(cluster) >= min_matching_events:
            activities_in_cluster = df.loc[cluster, 'ProcessedActivity'].value_counts()
            dominant_activity = activities_in_cluster.idxmax()
            ...
            if len(filtered_cluster) >= min_matching_events:
                ...
                for idx in filtered_cluster:
                    ...
                    processed_activity = df.at[idx, 'ProcessedActivity']
                    if original_activity.lower() == processed_activity.lower():
...
print(f"Clean events (unmodified): {clean_events}")
...
df_fixed.to_csv(output_file, columns=output_columns + ['Activity_fixed'], index=False)
...
removed_sample = df[df['is_collateral_event'] == 1].head(10)[[case_column, activity_column, timestamp_column]]
