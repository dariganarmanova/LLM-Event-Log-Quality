...
# Generated on: 2025-11-13T11:42:17.573433
...
df['ProcessedActivity'] = df['ProcessedActivity'].str.replace(homonymous_suffix, '', regex=False)
...
for cluster_id in df['cluster'].unique():
    ...
    if cluster_activities['ishomonymous'].sum() > 0:
        canonical = cluster_activities['BaseActivity'].value_counts().idxmax()
        cluster_to_canonical[cluster_id] = canonical
    else:
        canonical = cluster_activities['BaseActivity'].iloc[0]
        cluster_to_canonical[cluster_id] = canonical
