...
# Generated on: 2025-11-13T14:42:36.318964
...
try:
    ...
    for case_id, group in df.groupby(case_column):
        ...
        for i in range(n):
            ...
            cluster = [i]
            cluster_start_time = group.loc[i, timestamp_column]
            base_activity = group.loc[i, 'ProcessedActivity']
            mismatch_count = 0
            
            for j in range(i + 1, n):
                time_diff = (group.loc[j, timestamp_column] - cluster_start_time).total_seconds()
            ...
            valid_cluster = group.loc[cluster][group['ProcessedActivity'] == dominant_activity]
            
            if len(valid_cluster) >= min_matching_events:
                cluster_indices = valid_cluster.index
                unsuffixed = valid_cluster[valid_cluster['BaseActivity'] == dominant_activity]
                
                if not unsuffixed.empty:
                    keep_index = unsuffixed.index[0]
                else:
                    keep_index = valid_cluster.index[0]
                
                df.loc[cluster_indices, 'is_collateral_event'] = 1
                df.loc[keep_index, 'is_collateral_event'] = 0
                df.loc[cluster_indices, 'CollateralGroup'] = cluster_counter
                cluster_counter += 1
    ...
    print(f"Run 3: Processed dataset shape: {df_fixed.shape}")
    print(f"Run 3: Events removed: {len(df) - len(df_fixed)} ({(len(df) - len(df_fixed)) / len(df) * 100:.2f}%)")
